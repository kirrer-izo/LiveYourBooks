    ...
    public function generateAdvice(Request $request)
    {
        ...
        if ($request->thinker_id) {
            if (str_starts_with($request->thinker_id, 'thinker_')) {
                // Predefined thinker
                $thinkerTypeValue = str_replace('thinker_', '', $request->thinker_id);
                ...
                foreach ($authors as $author) {
                    if ('author_' . md5($author) === $request->thinker_id) {
                        $thinkerName = $author;
                        break;
                    }
                }
        ...
        $book = $request->book_id ? Book::where('user_id', $user->id)->find($request->book_id) : null;
        $goals = $request->goals ?? [];

        // If life area is specified, use it to generate goals
        if ($request->life_area) {
            $advice = $this->adviceCustomizer->generateLifeAreaAdvice($user, $request->life_area, $thinker, $thinkerName);
        } else {

...
const Chat = () => {
  ...
  const [showAddMentor, setShowAddMentor] = React.useState(false);
  const [newBookTitle, setNewBookTitle] = React.useState("");
  const [newBookAuthor, setNewBookAuthor] = React.useState("");
  const [newMentorName, setNewMentorName] = React.useState("");
  const [creatingBook, setCreatingBook] = React.useState(false);
  const [creatingMentor, setCreatingMentor] = React.useState(false);
  const [localBooks, setLocalBooks] = React.useState(books);
  const [localMentors, setLocalMentors] = React.useState(mentors);
  ...
  // Sync local books and mentors with props
  React.useEffect(() => {
    setLocalBooks(books);
  }, [books]);
  ...
  const handleCreateBook = async (e) => {
    ...
    try {
      const csrf = csrfToken();
      const res = await fetch('/api/quick-create-book', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
      ...
      // Add to local books list
      setLocalBooks([...localBooks, data.book]);
      setBookId(data.book.id.toString());
      setNewBookTitle("");
      setNewBookAuthor("");
  ...
  const handleCreateMentor = async (e) => {
    ...
    try {
      const csrf = csrfToken();
      const res = await fetch('/api/quick-create-mentor', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
      ...
      // Add to local mentors list
      const updatedMentors = [...localMentors, data.mentor.name].sort();
  ...
  return (
    <>
      ...
      <div className="space-y-4">
        ...
        {/* Mentor Selection Indicator */}
        {mentorName && (
          <div className="flex items-center gap-2 px-4 py-2 bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-200 dark:border-indigo-800 rounded-lg">
            <div className="w-2 h-2 bg-indigo-600 dark:bg-indigo-400 rounded-full animate-pulse"></div>
            <span className="text-sm font-medium text-indigo-700 dark:text-indigo-300">
              Chatting with: <span className="font-semibold">{mentorName}</span>
            </span>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="flex flex-col">
            ...
            {showAddBook && (
              <div className="mt-2 p-3 border border-gray-300 dark:border-gray-600 rounded bg-gray-50 dark:bg-gray-700">
                <form onSubmit={handleCreateBook} className="space-y-2">
                  <input
                    type="text"
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 px-2 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded text-sm disabled:opacity-50"
                    >
                      {creatingBook ? "Creating..." : "Add"}
                    </button>
                    <button
                      type="button"
                      onClick={() => {
          ...
          <div className="flex flex-col">
            ...
            {showAddMentor && (
              <div className="mt-2 p-3 border border-gray-300 dark:border-gray-600 rounded bg-gray-50 dark:bg-gray-700">
                <form onSubmit={handleCreateMentor} className="space-y-2">
                  <input
                    type="text"
              ...
              <div className="text-gray-500 dark:text-gray-400">
                {mentorName ? (
                  <div className="text-center space-y-2">
                    <div className="inline-block px-3 py-1 bg-indigo-100 dark:bg-indigo-900 text-indigo-700 dark:text-indigo-300 rounded-full text-sm font-medium">
                      Chatting with: {mentorName}
                    </div>
                ...
                {messages.map((m, i) => (
                  <div key={i}>
                    <div className={m.role === "user" ? "text-right" : "text-left"}>
                      {m.role === "assistant" && mentorName && (
                        <div className="text-xs text-gray-500 dark:text-gray-400 mb-1 ml-1">
                          {mentorName}
                        </div>
                      )}
                      <div className={`inline-block px-4 py-3 rounded-lg max-w-[85%] ${m.role === "user" ? "bg-indigo-600 text-white" : "bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white"}`}>
